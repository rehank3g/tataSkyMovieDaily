...
import requests
import json
import allContents as allContents
...
# This method gets the channelList from the allContents.json file and returns it as a list/dictionary
def getContentList():
    allContents.getAllContents()
    with open("allContents.json", "r") as savedChannelDetailInFile:
      savedContents = json.load(savedChannelDetailInFile)
...
def generateJWT(channelId, result,iterative=False):
    url = API_BASE_URL + "auth-service/v1/oauth/token-service/token"
    epidList=[{
                'epid': 'Subscription',
                'bid': result[channelId]["content_entitlements"][0]
    }]
    payload={
        'action': 'stream',
        'epids': epidList
    ...
    headers = getHeaders()
    x = requests.request("POST", url, headers=headers, data=jsonPayload)
